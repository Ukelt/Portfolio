...
<body>
    ...
    if ($row5['COUNT(DISTINCT `attempt_num`)'] < $quizMaxAttempts) {
        echo "<p>You have " . ($quizMaxAttempts - $row5['COUNT(DISTINCT `attempt_num`)'] ). " attempts remaining.</p>";
        echo "<button type='button' class='btn btn-primary' id='startQuiz'>Start Quiz</button>";
    } else {

        echo "<p>You have reached the maximum number of attempts for this quiz.</p>";
    ...
    $sql6 = "SELECT * FROM `quiz_attempt` WHERE `user_id` = ".$_SESSION['id']." AND `quiz_id` = $quizId";
    $result6 = mysqli_query($conn, $sql6);
    //check if num_rows is equal to the max attempts
    while ($row6 = mysqli_fetch_assoc($result6)) {

        echo "<p>Attempt Number: " . $row6['attempt_num'] . "</p>";
        echo "<p>Score: " . $row6['score'] . "</p>";
        //format the date to dd/mm/yyyy
        $date = date_create($row6['date_submitted']);
        $date = date_format($date, 'd/m/Y H:i:s');

        echo "<p>Attempt Date: " . $date . "</p>";
    //place a button on the right hand side of the page to view the results
    echo "<a href='./displayScore.php?id=".$row6['quiz_id']."&attempt=".$row6['attempt_num']."' class='btn btn-primary'>View Results</a>";
        echo "<hr>";
    }
    ...
    echo "<div class='container border rounded w-75 mt-3' id='quizContainer' style='display:none'>";
    ?>
    ...
    echo "<button type='submit' class='btn btn-primary' id='submitQuiz' name='submitQuiz'>Submit Quiz</button>";
    //save progress button
    echo "<button type='button' class='btn btn-primary' id='saveProgress'>Save Progress</button>";
    echo "</div>";
 ?>
    </form>
