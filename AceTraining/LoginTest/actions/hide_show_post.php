<?php 
    ...
    $sql = "SELECT `start_date`, `end_date` FROM `course_posts` WHERE post_id = '$post_id'";
    ...
    if($start_date <= date("Y-m-d m:s:i") && $end_date >= date("Y-m-d m:s:i")){
        ...
        //hide post
        $date = "2000-01-01";
        $sql = "UPDATE `course_posts` SET `end_date` = '$date' WHERE post_id = '$post_id'";
        ...
        if($start_date > date("Y-m-d m:s:i")){
            ...
            $date = date("Y-m-d");
            $sql = "UPDATE `course_posts` SET `start_date` = '$date' WHERE post_id = '$post_id'";
            $result = mysqli_query($conn, $sql);
            var_dump($sql);
            var_dump($result);
        }
